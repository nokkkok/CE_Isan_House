...
class Booking:
    def __init__(self, booking_id: str, customer: 'Customer', showtime: 'Showtime', 
                 seats: list = None, timestamp: str = None, status: str = "Pending", 
                 total_price: float = 0):
        ...
        if total_price == 0 and seats:
            # Simple calculation: $10 per seat
            self.__total_price = len(seats) * 10.0
    ...
    def add_seat(self, seat_id):
        ...
        if seat_id not in self.__seats:
            self.__seats.append(seat_id)
            # Update price when adding a seat
            self.__total_price += 10.0
            return True
        return False
            
    def remove_seat(self, seat_id):
        """Remove a seat from the booking"""
        if seat_id in self.__seats:
            self.__seats.remove(seat_id)
            # Update price when removing a seat
            self.__total_price -= 10.0
            return True
        return False
    ...
    def calculate_total_price(self):
        ...
        food_total = sum(food_order.calculate_subtotal() for food_order in self.__food_list)
...
class Regular(Customer):
    def upgrade_membership(self, points=0):
        return Member(self.customer_id, self.name, self.email, self.__password, points)
    
class Member(Customer):
    def __init__(self, customer_id: str, name: str, email: str, password: str, points: int = 0):
